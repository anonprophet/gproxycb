...
QByteArray CGPSProtocol :: SEND_GPSC_INIT( quint32 version )
{
	...
	packet.push_back( GPS_INIT );
        ...
        packet.push_back((char)0);
        Util::AppendUInt32(packet,version);
	AssignLength( packet );
	return packet;
}
...
QByteArray CGPSProtocol :: SEND_GPSC_RECONNECT( unsigned char PID, quint32 reconnectKey, quint32 lastPacket )
{
	...
	packet.push_back( PID );
        Util::AppendUInt32( packet, reconnectKey);
        Util::AppendUInt32( packet, lastPacket );
	AssignLength( packet );
	return packet;
}
...
QByteArray CGPSProtocol :: SEND_GPSS_INIT( quint16 reconnectPort, unsigned char PID, quint32 reconnectKey, unsigned char numEmptyActions )
{
	...
	packet.push_back( GPS_INIT );
        packet.push_back((char)0);
        packet.push_back((char)0);
        Util::AppendUInt32 (packet, reconnectPort );
	packet.push_back( PID );
        Util::AppendUInt32 (packet, reconnectKey);
...
QByteArray CGPSProtocol :: SEND_GPSS_REJECT( quint32 reason )
{
	...
	packet.push_back( GPS_REJECT );
        packet.push_back((char)0);
        packet.push_back((char)0);
        Util::AppendUInt32( packet, reason);
	AssignLength( packet );
	return packet;
}
